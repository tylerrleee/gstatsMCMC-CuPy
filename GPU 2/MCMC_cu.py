...
class chain_sgs_gpu(chain):
    ...
    def run(self, n_iter, only_save_last_bed=False, info_per_iter=100, plot=True, progress_bar=True):
        ...
        if self.detrend_map == True:
            mc_res = get_mass_conservation_residual_GPU(bed_c + self.trend, self.surf, self.velx, self.vely, self.dhdt, self.smb, resolution)
        else:
            mc_res = get_mass_conservation_residual_GPU(bed_c, self.surf, self.velx, self.vely, self.dhdt, self.smb, resolution)
        ...
        rand_idx = rng.integers(0, n_valid, size=n_iter)
        ...
        for i in iterator:
            ...
            local_mc_res_pad = get_mass_conservation_residual_GPU(
